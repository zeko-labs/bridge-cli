...
const normalizeRouteString = (route: string): string | null => {
	try {
		const [from, to] = parseRouteString(route)
		return formatRoute(from, to)
	} catch {
		return null
	}
}
...
export const matchesPendingItem = ({
	...
	normalizeRouteString(session.route) === formatRoute(from, to) &&
