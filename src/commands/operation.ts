...
const createAdapterWithRetries = async ({
	...
	for (let attempt = 1; ; attempt += 1) {
		try {
			return await createAdapter({ from, to, verbose })
		} catch (error) {
			if (!isRetryableInitializationError(error)) {
				throw error
			}
...
const resolveRetryDelayMs = (input: {
	...
	fallback?: number
}): number => input.argValue ?? input.deps.retryDelayMs ?? input.fallback ?? 5_000
	...
	defineCommand({
		...
		subCommands: {
			...
			"resume-all": defineCommand({
				...
				async run({ args }) {
					const from = parseChainArg(args.from, "mina:testnet", "--from")
					const to = parseChainArg(args.to, "zeko:testnet", "--to")
					const json = readBooleanArg(args, "json")
					const verbose = readBooleanArg(args, "verbose")
